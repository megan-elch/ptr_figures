...
load("model_output/suff_stats_testes.RData")
group_ref = protein_suff %>% 
  dplyr::group_by(UNIPROT, pop_protein, ct) %>%
  dplyr::summarise(mean_prot = mean(pep_av, na.rm = T)) %>%
  ungroup() %>%
  dplyr::group_by(UNIPROT, pop_protein) %>%
  dplyr::summarise(var_ct = var(mean_prot, na.rm = T),
                   n_ct = length(unique(ct))) %>%
  ungroup() %>%
  filter(n_ct >= 5) %>%
  dplyr::group_by(UNIPROT) %>%
  dplyr::summarise(av_var = mean(var_ct, na.rm = T)) %>%
  ungroup() %>%
  dplyr::mutate(var_group_n = ntile(av_var, 3),
                var_group = factor(var_group_n)) %>%
  mutate(var_group = fct_recode(var_group, "Low Variance" = "1", "Moderate Variance" = "2", "High Variance" = "3"))
